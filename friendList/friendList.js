...
const friendSearch = document.querySelector(".container .search input");

function updateFriendList() {
  const searchText = friendSearch.value.toLowerCase();

  const updateFriend = friendsData.filter((friend) =>
    friend.name.toLowerCase().includes(searchText)
  );

  box.innerHTML = "";

  updateFriend.forEach((friend, index) => {
    const listItem = createFriend(
      friend.name,
...
friendSearch.addEventListener("input", updateFriendList);
